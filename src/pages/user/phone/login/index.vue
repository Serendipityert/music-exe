<template>
    <div class="mt-8 flex flex-col">
        <div class="mb-4 text-sm font-bold">手机号登录</div>
        <div>
            <t-form ref="form" :data="formData" :rules="rules" :colon="true" :label-width="0" @submit="userLogin">
...
const userLogin = ({ validateResult, firstError }) => {
    if (validateResult === true) {
        ...
        userPhoneLogin(formData.value).then((res: any) => {
            if (res.code === 1) {
                MessagePlugin.success(res.msg)
                userState.setUserInfo(res.data.User)
                userState.setToken(res.data.token)
                // 返回父组件登录成功
                userState.setIsLogin(true)
                
                emit('userLogin', userState.getIsLogin)
                // 重置表单
                onRestForm()
            } else {
