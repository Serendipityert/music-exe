<template>
  <div class="flex flex-col">
    ...
    <div class="flex flex-col">
      <div class="font-medium text-xl mt-6 text-gray-800">
        Hi {{ userInfo.profile.nickname }} 今日为你推荐
      </div>
      <div class="flex flex-around mt-2">
        <div
          class="w-96 h-40 cursor-pointer rounded-xl mr-4 p-2 text-center img"
          @click="openRecommendDj">
          <img
            v-if="recommendDj"
            :src="recommendDj[recommendDj.index].picUrl"
            class="rounded-xl mb-2"
            style="width: 100%; height: 100%" />
          <div class="text-xs mt-2 cursor-pointer hover:text-blue-800">
            个性电台
          </div>
        </div>
        <div
          class="w-60 h-40 cursor-pointer rounded-xl mr-4 p-2 text-center img"
          @click="openRecommendedSongsList">
          <img
            v-if="recommendedSongsList"
            :src="
              recommendedSongsList.dailySongs[recommendedSongsList.index].al
                .picUrl
            "
            class="rounded-xl mb-2"
            style="width: 100%; height: 100%" />
        ...
        <div
          class="w-60 h-40 cursor-pointer rounded-xl mr-4 p-2 text-center img">
          <img
            v-if="recommendNewSongs"
            :src="recommendNewSongs[recommendNewSongs.index].picUrl"
            class="rounded-xl mb-2"
            style="width: 100%; height: 100%" />
        ...
        <div class="w-60 h-40 cursor-pointer rounded-xl p-2 text-center img">
          <img
            v-if="recommendMv"
            :src="recommendMv[recommendMv.index].picUrl"
            class="rounded-xl mb-2"
            style="width: 100%; height: 100%" />
    ...
    <div class="flex flex-col">
      ...
      <div class="flex flex-col mt-2">
        <div class="flex flex-around">
          <div
            class="w-60 h-40 cursor-pointer rounded-xl mr-2 p-1 img"
            v-if="recommendedPlayList"
            v-for="(item, index) in recommendedPlayList"
            :key="index"
            @click="openRecommendedPlayList(item)">
            <img :src="item.picUrl" alt="" style="width: 100%; height: 90%" />
            <div class="text-xs w-50 mt-2 hover:text-blue-800 cursor-pointer">
              {{ item.name }}
            </div>
          </div>
        </div>
    ...
    <div class="flex flex-col">
      ...
      <div class="flex flex-wrap content-start mt-2 ml-2">
        <div
          class="w-36 h-36 cursor-pointer rounded-xl mr-2 p-1 mb-1 text-center img"
          v-if="hotSongerList"
...
getDayRecommendSongs(cookie.value).then((res: any) => {
  if (res.code === 200) {
    ...
    recommendedSongsList.value.index = getRandomNumber(
      res.data.dailySongs.length
    );
  }
});
